...
# stream from camera
  gst-launch-1.0 rtspsrc location=rtsp://root:pass@${1}/axis-media/media.amp latency=0 name=src ! queue ! rtph264depay ! queue ! h264parse ! flvmux streamable=true name=mux ! rtmpsink location="${2}" src. ! decodebin ! audioconvert ! audioresample ! audio/x-raw,rate=48000 ! voaacenc bitrate=96000 ! audio/mpeg ! aacparse ! audio/mpeg, mpegversion=4 ! mux.
